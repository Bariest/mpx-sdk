...
def cmd_init(args: argparse.Namespace) -> None:
    ...
    makefile = (
        ...
        f"# Override the robot address: make deploy MPX_HOST=10.0.0.5\n"
        f"export MPX_HOST ?= 192.168.2.1\n"
        f"SRC := src/{name}.{ext}\n"
        f"WASM := build/{name}.wasm\n\n"
        f".PHONY: deploy build validate upload run clean\n\n"
        f"# The usual loop: build, upload and run in one step. Paths come from\n"
        f"# manifest.json, so nothing here needs the skill name.\n"
        f"deploy:\n"
        f"\tmpx-cli deploy\n\n"
        f"build:\n"
        f"\tmpx-cli build\n\n"
        f"validate:\n"
        f"\tmpx-cli build --validate\n\n"
        f"upload:\n"
        f"\tmpx-cli upload\n\n"
        f"run:\n"
        f"\tmpx-cli run\n\n"
